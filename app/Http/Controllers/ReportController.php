...
//            -----------------------------
    ...
    //         $invoiceData = Invoice:: selectRaw('MONTH(send_date) as month,YEAR(send_date) as year,category_id,invoice_id,id');
    //         $invoiceData->where('created_by', \Auth::user()->creatorId());
    //         $invoiceData->where('status', '!=', 0);
    //         $invoiceData->whereRAW('YEAR(send_date) =?', [$year]);
    //         $invoiceData->groupBy('month', 'year');
    //         $invoiceData = $invoiceData->get();
    //         $invoiceArr   = [];
    //         foreach($invoiceData as  $invoice)
    //         {
    //             $invoiceArr[$invoice->month] = $invoice->getTotal();
    //         }

    //         // ------------------------------TOTAL PAYMENT-----------------------------------------------------------
    //         $paymentsData = Transaction::selectRaw('sum(transactions.amount) as amount,MONTH(date) as month,YEAR(date) as year');
    //         $paymentsData->where('transactions.type','payment');
    //         $paymentsData->where('transactions.created_by', '=', \Auth::user()->creatorId());
    //         $paymentsData->whereRAW('YEAR(date) =?', [$year]);
    //         $paymentsData->groupBy('month', 'year');
    //         $paymentsData = $paymentsData->get();
    //         $paymentArr   = [];
    //         foreach($paymentsData as $k => $paymentData)
    //         {
    //             $paymentArr[$paymentData->month] = $paymentData->amount;
    //         }
    ...
    //         for($i = 1; $i <= 12; $i++)
    //         {
    //             $InvoiceTotal[] = array_key_exists($i, $invoiceArr) ? $invoiceArr[$i] : 0;
    //             $PaymentTotal[] = array_key_exists($i, $paymentArr) ? $paymentArr[$i] : 0;
    ...
    //         foreach($keys as $v)
    //         {
    //             $profits[$v] = (empty($totalIncome[$v]) ? 0 : $totalIncome[$v]) - (empty($totalExpense[$v]) ? 0 : $totalExpense[$v]);
    //         }
    ...
    //         return view('report.profit_loss', compact('filter'),  $data);
    //     }
    //     else
    //     {
    //         return redirect()->back()->with('error', __('Permission denied.'));
